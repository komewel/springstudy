...
public class MovieServiceImpl implements MovieService {
	...
	public Map<String, Object> getAllMovies() {
		List<MovieDTO> list = movieMapper.getAllMovies(); // 전체목록 준비
		int status = 0;
		String message = null;
		if(list.isEmpty()) {
			status = 500;
	...
	@Override
	public Map<String, Object> getMoviesQuery(QueryDTO queryDTO) {
		
		queryDTO.setSearchText(securityUtil.preventXSS(queryDTO.getSearchText()));  
		
		List<MovieDTO> list = movieMapper.getMoviesByQuery(queryDTO); // 목록 가져옴
		int status = 0;
		String message = null;
		if(list.isEmpty()) {
			status = 500;
