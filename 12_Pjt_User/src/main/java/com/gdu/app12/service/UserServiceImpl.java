...
public class UserServiceImpl implements UserService {
	  ...
	  public void join(HttpServletRequest request, HttpServletResponse response) {
	    ...
	    String birthdate = request.getParameter("birthdate"); // 01 이런식으로 표현해주려면 int로는 안되고 String처리를 해야한다.
	...
	public void autologin(HttpServletRequest request, HttpServletResponse response) {
		  ...
		  /*
			...
			1. 자동 로그인을 체크한 경우
			  1) session의 id를 DB의 AUTOLOGIN_ID에 저장한다.(본인의 아이디로 로그인 하는게 아니다, 비밀번호의 입력도 없다, 고로 아이디 자체가 나만 알고있는 아이디가 존재해야한다. 중복이 없고(UUID) 다른사람이 알기 어려운 정보를 이용해서 자동 로그인에서 사용할 ID를 결정한다.)
			  2) 자동 로그인을 유지할 기간(예시: 15일)을 DB의 AUTOLOGIN_EXPIRED_AT 칼럼에 저장한다.
			  3) session의 id를 쿠키로 저장한다. (쿠키: 각 사용자의 브라우저에 저장되는 정보, 보안에는 취약할수 있다)
			   	  이 때 쿠키의 유지 시간을 자동 로그인을 유지할 기간과 동일하게 맞춘다.
			  		
			2. 자동 로그인을 체크하지 않은 경우(지우는걸 위주로 하면 된다)
		  ...
		  if(chkAutoLogin != null) {
			  ...
			  userDTO.setAutologinExpiredAt(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * 24 * 15)); 
			  									   ...
			  									   // java.sel.Date 클래스는 타임스탬프를 이용해서 날짜를 생성한다. 
			  ...
			  cookie.setPath(request.getContextPath()); //쿠키의 경로를 contextPathfh 설정해서 어디서든 확인할수 있게 해준다, 애플리케이션의 모든 URL에서 autoLoginId 쿠키를 확인할 수 있다
			  response.addCookie(cookie); // 서버가 쿠키를 만들어서 브라우저(사용자)에게 보낸다(응답), 고로 쿠키저장은 응답(response)이 한다.
		  ...
		  // 자동 로그인을 체크하지 않은 경우
			  ...
			  // autoLoginId 쿠키 삭제하기
			  Cookie cookie = new Cookie("autoLoginId", ""); // 삭제할것이므로 쿠키값이 있을 필요는 없다
			  cookie.setMaxAge(0); // 쿠키 유지시간을 0초로 설정, 삭제하는법
