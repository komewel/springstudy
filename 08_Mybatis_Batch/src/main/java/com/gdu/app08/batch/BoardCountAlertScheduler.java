...
3. 크론식
	...
	4) 작성 예시
		...
		(4) 매일 3시간마다      : 0 0 0/3 * * ?
		(5) 월요일 12시마다     : 0 0 12 ? * 1(MON이라는 뜻) 또는 0 0 12 ? * MON
...
public class BoardCountAlertScheduler { // 따로 bean화 시켜서 @EnableScheduling 설정을 해줘도 된다, 그렇지만 @Component로 간단하게 bean화 시켜도된다(더 간단하다, 사실상 따로 만들 이유가 없기 때문에) 

	@Autowired
	private BoardService boardService;
	
	@Scheduled(cron = "0 0/1 * * * ?") // 스케쥴 타임을 적는 방식, 크론식(1분마다)
	public void execute() {
