...
public class UploadServiceImpl implements UploadService {
	...
	public int addUpload(MultipartHttpServletRequest multipartRequest) {
		...
		// 첨부가 없는 경우에도 files 리스트는 비어있지 않고,
		// 
	...
	public ResponseEntity<Resource> downloadAll(int uploadNo) {
		...
		// 모든 첨부 파일을 zip 파일로 압축해서 다운로드 하는 서비스(이 파일을 어디다 어떤 이름으로 저장하느냐)
		// com.gdu.batch.RemoveTempfileScheduler에 의해서 주기적으로 zip 파일들은 삭제된다.
		...
		try {
			...
			for(AttachDTO attachDTO : attachList) { // attachList에 있는 정보를 하나씩 빼서 AttachDTO에 저장한다.
				...
				byte[] b = new byte[1024];  // 첨부 파일을 1KB(1024 byte) 단위로 읽겠다.
				int readByte = 0;		    // 실제로 읽어 들이는 바이트 수 (ex. 1025byte 같은 경우는 한번에 못 읽는다)
				while((readByte = bin.read(b)) != -1) {
					zout.write(b, 0, readByte); // b를 0(처음) 부터 읽어들인 데이터(readByte) 만큼만 
				}
				bin.close();
				zout.closeEntry();
				
				// bin -> zout으로 파일 복사하기 (Spring코드)
				// FileCopyUtils.copy(bin, zout); ,위는 자바 아래는 스프링
