...
-- 1게시글 - 1첨부 - 1테이블
-- 1게시글 - 다중첨부 - 2테이블(이걸로 연습 할거, 1 대 다 관계이다.)
...
CREATE TABLE ATTACH (
    ...
    FILESYSTEM_NAME  VARCHAR2(50 BYTE) NOT NULL,  -- 첨부파일의 저장 이름
    DOWNLOAD_COUNT   NUMBER,                      -- 다운로드 횟수
    HAS_THUMBNAIL    NUMBER,                      -- 썸네일이 있으면 1, 없으면 2
    UPLOAD_NO        NUMBER                       -- 게시글 FK(이렇게 달아줘도 된다)
);
...
CREATE TABLE UPLOAD (
    ...
    UP_TITLE       VARCHAR2(1000 BYTE) NOT NULL, -- 제목
